...
AGGREGATION_PATTERNS = [
    r"total\s+\w+\s+(spent|earned|cost|paid|revenue|income|budget)",
    r"sum\s+of\s+\w+",
    r"how\s+much\s+\w+\s+(spent|earned|cost|paid|revenue|income|budget)",
    r"how\s+much\s+(was|were|is|are)\s+(spent|earned|paid|budgeted|allocated)",
    r"average\s+(spending|revenue|cost|income|salary|price|amount)",
    r"combined\s+(spending|revenue|cost|income|budget|total)",
    r"(last|past|previous)\s+\d+\s+(months?|years?|quarters?)\s+.*(spending|revenue|cost|income|total)",
    r"(q[1-4]|quarter)\s*\d*\s+.*(spending|revenue|cost|income)",
    r"(fy|fiscal\s+year)\s*\d+\s+.*(spending|revenue|cost|income)",
]
