...
MAX_FOLDER_DEPTH = 20  # Prevent unbounded nesting

# Per-request session scoping: ensures all methods within one request
# share the same session, while different requests are isolated.
_active_session: ContextVar[Optional[AsyncSession]] = ContextVar(
    '_folder_active_session', default=None
)
...
class FolderService:
    ...
    async def _session_scope(self) -> AsyncIterator[AsyncSession]:
        """Provide a properly-managed database session.

        - Reuses injected session (caller manages lifecycle)
        - Reuses existing scope session (nested calls share one session)
        - Creates and closes a fresh session otherwise
        """
        if self._injected_session:
            yield self._injected_session
            return

        existing = _active_session.get(None)
    ...
    async def get_folder_by_id(
        ...
        async with self._session_scope() as session:
            ...
            if organization_id:
                query = query.where(Folder.organization_id == uuid.UUID(organization_id))
    ...
    async def _update_descendant_paths(self, old_prefix: str, new_prefix: str) -> int:
        ...
        async with self._session_scope() as session:
            ...
            # Commit deferred to caller's session scope
            return count
    ...
    async def move_folder(
        ...
        """
        async with self._session_scope() as session:
            folder = await self.get_folder_by_id(folder_id, organization_id=organization_id)
            if not folder:
                return None

            # Check if moving to a descendant (invalid)
            if new_parent_id:
                new_parent = await self.get_folder_by_id(new_parent_id, organization_id=organization_id)
                if not new_parent:
            ...
            # Compute new path and depth (reuse new_parent from validation above)
            if new_parent_id:
